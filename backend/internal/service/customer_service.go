...
func (s *CustomerService) CreateCustomer(ctx context.Context, customer *domain.Customer) error {
	...
	// Check for duplicate by contact number if provided
	if customer.ContactNumber != "" {
		...
		if existing != nil {
			return &domain.BusinessError{
				...
				Message: fmt.Sprintf("customer with contact number %s already exists", customer.ContactNumber),
			}
		}
	}
...
func (s *CustomerService) UpdateCustomerBalance(ctx context.Context, customerID string, amount float64, transactionType string) error {
	delta := amount
